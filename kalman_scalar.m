...
sigmaEtaModel=0.01; %ошибка измерений прибора
kk = zeros(1, K); %номера отсчетов для визуализации графиков 

%ковариационная матрица шума системы (параметров)
%Rpr = [0.3 0 0 0; 0 0.021 0 0; 0 0 0.006 0; 0 0 0 0.1]; %подобрать дисперсию для последнего пар-ра
...
for i = 1:50 % 50 экспериментов (т.к. сигнал стохастический)
    ...
    devPhi(i) = var(ph_init_est); %std - это стандартное отклонение, var - дисперсия
...
figure(1);
set(gcf,'color','w');
boxplot(DEV_A, T_vec), xlabel('Количество отсчётов на период сигнала'), ylabel('Значение погрешности, отн.ед.');
figure(2);
set(gcf,'color','w');
boxplot(DEV_B, T_vec), xlabel('Количество отсчётов на период сигнала'), ylabel('Значение погрешности, отн.ед.');
figure(3);
set(gcf,'color','w');
boxplot(DEV_PHI, T_vec), xlabel('Количество отсчётов на период сигнала'), ylabel('Значение погрешности, радиан');
figure(4);
set(gcf,'color','w');
boxplot(DEV_T, T_vec), xlabel('Количество отсчётов на период сигнала'), ylabel('Значение погрешности, отн.ед.');
%plot(B, k);
%figure (2),
